...
(() => {
	...
	const F = (window.__fake = {
		log: [],
		calendars: {
			primary: { id: 'primary', summary: 'tester@example.com', backgroundColor: '#3B7DD8', primary: true },
			'work@group.calendar.google.com': { id: 'work@group.calendar.google.com', summary: 'Work', backgroundColor: '#D85B8B' },
			'home@group.calendar.google.com': { id: 'home@group.calendar.google.com', summary: 'Home', backgroundColor: '#33B679' },
		},
	...
	const addEvent = (c, ev) => {
		const e = { id: id('evt'), etag: `"${seq++}"`, status: 'confirmed', ...ev };
		cal(c)[e.id] = e;
		pushChange(c, e);
		return e;
	};
	...
	const d = (off, h) => {
		const x = new Date(today.getFullYear(), today.getMonth(), today.getDate() + off, h ?? 0, 0, 0);
		return h === undefined ? x.toISOString().slice(0, 10) : x.toISOString();
	};
	addEvent('primary', { summary: 'Weekly meeting', start: { dateTime: d(1, 10) }, end: { dateTime: d(1, 11) }, description: 'agenda' });
	addEvent('primary', { summary: 'Recurring standup', start: { dateTime: d(2, 9) }, end: { dateTime: d(2, 9.5) }, recurringEventId: 'rec1' });
	addEvent('work@group.calendar.google.com', { summary: 'Offsite', start: { date: d(3) }, end: { date: d(5) } });
	addEvent('home@group.calendar.google.com', { summary: 'Dentist', start: { dateTime: d(-1, 14) }, end: { dateTime: d(-1, 15) } });
	cal('primary');

	const err = (status, message) => ({ status, text: JSON.stringify({ error: { code: status, message } }), headers: {} });
	const ok = (body) => ({ status: body === undefined ? 204 : 200, text: body === undefined ? '' : JSON.stringify(body), headers: {} });
	...
	function route(method, urlStr, opts) {
		...
		let m;
		if (p === '/calendar/v3/users/me/calendarList') return ok({ items: Object.values(F.calendars) });
		if ((m = p.match(/^\/calendar\/v3\/calendars\/([^/]+)\/events$/))) {
			...
			if (method === 'GET') {
				...
				if (tok) {
					...
					return ok({ items: F.changes[c].slice(from), nextSyncToken: `st${F.changes[c].length}` });
				}
				if (!q.get('timeMin') || q.get('singleEvents') !== 'true') return err(400, 'full sync must pass timeMin and singleEvents');
				return ok({ items: Object.values(store), nextSyncToken: `st${F.changes[c].length}` });
			}
			if (method === 'POST') {
				if (!body.summary || !body.start || !body.end) return err(400, 'summary/start/end required');
		...
		if ((m = p.match(/^\/tasks\/v1\/lists\/([^/]+)\/tasks$/))) {
			...
			if (method === 'GET') {
				if (q.get('showHidden') !== 'true' || q.get('showCompleted') !== 'true') return err(400, 'expected showHidden/showCompleted');
				return ok({ items: Object.values(F.tasks[l]) });
			}
			const t = { id: id('task'), status: 'needsAction', updated: now(), ...body };
			F.tasks[l][t.id] = t;
			return ok(t);
		}
	...
	plugin.google.send = async (method, url, token, opts) => {
		...
		F.log.push({ method, url: url.replace('https://www.googleapis.com', '').replace('https://tasks.googleapis.com', ''), status: res.status });
		return res;
	};
